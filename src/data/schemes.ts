export interface Scheme {
  ...
  process: { en: string[]; hi: string[] };
  // Eligibility criteria for checker
  criteria: {
    minAge?: number;
    maxAge?: number;
    maxIncome?: number;
...
export const schemes: Scheme[] = [
  ...
  {
    id: 'pm-awas',
    name: { en: 'PM Awas Yojana', hi: 'पीएम आवास योजना' },
    category: 'housing',
    description: {
...
export function checkEligibility(userProfile: {
  ...
  category: string;
  occupation: string;
}): { eligible: Scheme[]; notEligible: { scheme: Scheme; reasons: { en: string; hi: string }[] }[] } {
  const eligible: Scheme[] = [];
  const notEligible: { scheme: Scheme; reasons: { en: string; hi: string }[] }[] = [];
  ...
  for (const scheme of schemes) {
    ...
    if (c.minAge && userProfile.age < c.minAge) {
      reasons.push({ en: `Must be at least ${c.minAge} years old`, hi: `कम से कम ${c.minAge} वर्ष की उम्र होनी चाहिए` });
    }
    if (c.maxAge && userProfile.age > c.maxAge) {
      reasons.push({ en: `Must be under ${c.maxAge} years old`, hi: `${c.maxAge} वर्ष से कम उम्र होनी चाहिए` });
    }
    if (c.maxIncome && userProfile.income > c.maxIncome) {
      reasons.push({ en: `Annual income must be below ₹${c.maxIncome.toLocaleString()}`, hi: `वार्षिक आय ₹${c.maxIncome.toLocaleString()} से कम होनी चाहिए` });
    }
    if (c.gender && !c.gender.includes(userProfile.gender)) {
      reasons.push({ en: `Only for ${c.gender.join('/')}`, hi: `केवल ${c.gender.join('/')} के लिए` });
    }
    if (c.occupations && !c.occupations.includes(userProfile.occupation)) {
      reasons.push({ en: `Must be a ${c.occupations.join('/')}`, hi: `${c.occupations.join('/')} होना चाहिए` });
    }
